...
def plot_route_colored_by_descriptor(df: pd.DataFrame, descriptor: str, path: Path) -> Path:
    ...
    plt.colorbar(sc, label=f"{descriptor} (colores)")
    plt.xlabel("Longitud")
    plt.ylabel("Latitud")
    plt.title(f"Ruta coloreada por {descriptor}")
    plt.suptitle(f"Unidad: {unidad} | N puntos={n_points} | Color: {descriptor}", fontsize=9)
    plt.tight_layout()
...
def plot_route_colored_by_class(df: pd.DataFrame, path: Path) -> Path:
    ...
    plt.suptitle(f"Unidad: {unidad} | N puntos={n_points} | Color: Clase YAMNet dominante", fontsize=9)
    plt.tight_layout()
    plt.savefig(path, dpi=150)
    plt.close()
    return path
